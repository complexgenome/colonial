...
# -- load modules 
module load trimmomatic quast python2.7 biopython/1.63 
module load spades/3.10.0
# bio python is used in python coverage assembly script..

#
coverage="5" # default - changed on Feb 10 2017 
...
do
	...
	then 
	    ...
	    job_id=$(sbatch  /home/sariyasanjeev/scripts/assembly/310spades/automate_process.sh -f $forward_link -r $reverse_link -t $num_threads -o $out_dir/$i -a $adap_file -c $coverage  -s $tasks -w $script_dir | awk '{print $4}')
