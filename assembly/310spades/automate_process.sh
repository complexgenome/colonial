...
#SBATCH --time=14-0:0:0  # - time to run          #changed on May 17 from 4 to 7 
#SBATCH -o "main_automate.%j.out"  # output 
#SBATCH -J spades_trimmo # job name 
#SBATCH  -p 256gb # partition to run 
...
module load python2.7 samtools/0.1.18 bowtie2/2.2.3 spades/3.10.0
...
# $1 is dir of trimmed reads # $2 forward trimmed reads  # $3 reverse trimmed reads 
    ...
    delim='_' # this is for Kleb, staph
    #delim='-' # this is for ecoli

    #mlst_db="Klebsiella_pneumoniae"
    #mlst_db="wzi_seq"
    mlst_db="Staphylococcus_aureus" # do not add fasta to it. PLEASE -- 
    #mlst_db="Escherichia_coli#1" # do not add fasta to it. PLEASE || use this for E.coli

    #mlst_def="wzi_seq_profile.txt"
    #mlst_def="kpneumoniae.txt"
    mlst_def="saureus.txt"
    #mlst_def="ecoli.txt" # || for e.coli
...
# it runs mismatch corrector with --careful flag <which is recommended> # Default gzip output # Default runs bayes_hammer for read correction and assembler 
    ...
    /c1/apps/spades/SPAdes-3.10.0-Linux/bin/spades.py -t $num_threads --cov-cutoff $cut_off --careful -1 $out_dir/"out_paired_"$temp_forw_read -2 $out_dir/"out_paired_"$temp_rev_read -o $out_dir
    ...
    rm $out_dir/"out_unpaired_"$temp_forw_read $out_dir/"out_unpaired_"$temp_rev_read $out_dir/"before_rr.fasta" $out_dir/"before_rr.fastg" $out_dir/"out_paired_"$temp_forw_read $out_dir/"out_paired_"$temp_rev_read
    #remove un-paired and paired ends generated by trimmmomatic
